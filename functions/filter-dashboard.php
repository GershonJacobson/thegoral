...
if(isset( $_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
	...
	if($selectedFilter == "at" || $selectedFilter == "") {
		...
		$qAccumulateTicketBought = mysqli_query($con, "SELECT COALESCE(SUM(total_ticket), 0) AS total_ticket FROM tbl_ticket WHERE purchased_by = '" . $getUserID . "'");
		...
		$qRafflesWon = mysqli_query($con, "SELECT ROUND(COALESCE(SUM(pot.total), 0) / 2, 2) AS total_won FROM tbl_ticket w JOIN (SELECT campaignid_fk, SUM(total_price) AS total FROM tbl_ticket GROUP BY campaignid_fk) pot ON pot.campaignid_fk = w.campaignid_fk WHERE w.purchased_by = '" . $getUserID . "' AND w.win = 'Y'");
		$dRafflesWon = mysqli_fetch_array($qRafflesWon);
		$totalRafflesWon = $dRafflesWon['total_won'];
		
		$qTicketPurchased = mysqli_query($con, "
			SELECT campaignid_fk, DATE_FORMAT(purchased_date,'%m/%d/%Y') AS purchased_date, total_price, card_last4 AS card_number, payment_status FROM tbl_ticket WHERE purchased_by = '$getUserID' ORDER BY purchased_date DESC LIMIT 5
		");
	}
		...
		$qAccumulateTicketBought = mysqli_query($con, "SELECT COALESCE(SUM(total_ticket), 0) AS total_ticket FROM tbl_ticket WHERE purchased_by = '" . $getUserID . "' AND DATE_FORMAT(purchased_date,'%m') = '$currentMonth'");
		$dAccumulateTicketBought = mysqli_fetch_array($qAccumulateTicketBought);
		$totalTicket = $dAccumulateTicketBought['total_ticket'];
		...
		$qRafflesWon = mysqli_query($con, "SELECT ROUND(COALESCE(SUM(pot.total), 0) / 2, 2) AS total_won FROM tbl_ticket w JOIN (SELECT campaignid_fk, SUM(total_price) AS total FROM tbl_ticket GROUP BY campaignid_fk) pot ON pot.campaignid_fk = w.campaignid_fk WHERE w.purchased_by = '" . $getUserID . "' AND w.win = 'Y' AND DATE_FORMAT(w.purchased_date,'%m') = '$currentMonth'");
		$dRafflesWon = mysqli_fetch_array($qRafflesWon);
		$totalRafflesWon = $dRafflesWon['total_won'];
		
		$qTicketPurchased = mysqli_query($con, "
			SELECT campaignid_fk, DATE_FORMAT(purchased_date,'%m/%d/%Y') AS purchased_date, total_price, card_last4 AS card_number, payment_status FROM tbl_ticket WHERE purchased_by = '$getUserID' AND DATE_FORMAT(purchased_date,'%m') = '$currentMonth' ORDER BY purchased_date DESC LIMIT 5
		");
	}
		...
		$qAccumulateTicketBought = mysqli_query($con, "SELECT COALESCE(SUM(total_ticket), 0) AS total_ticket FROM tbl_ticket WHERE purchased_by = '" . $getUserID . "' AND DATE_FORMAT(purchased_date,'%Y') = '$currentYear'");
		$dAccumulateTicketBought = mysqli_fetch_array($qAccumulateTicketBought);
		$totalTicket = $dAccumulateTicketBought['total_ticket'];
		...
		$qRafflesWon = mysqli_query($con, "SELECT ROUND(COALESCE(SUM(pot.total), 0) / 2, 2) AS total_won FROM tbl_ticket w JOIN (SELECT campaignid_fk, SUM(total_price) AS total FROM tbl_ticket GROUP BY campaignid_fk) pot ON pot.campaignid_fk = w.campaignid_fk WHERE w.purchased_by = '" . $getUserID . "' AND w.win = 'Y' AND DATE_FORMAT(w.purchased_date,'%Y') = '$currentYear'");
		$dRafflesWon = mysqli_fetch_array($qRafflesWon);
		$totalRafflesWon = $dRafflesWon['total_won'];
		
		$qTicketPurchased = mysqli_query($con, "
			SELECT campaignid_fk, DATE_FORMAT(purchased_date,'%m/%d/%Y') AS purchased_date, total_price, card_last4 AS card_number, payment_status FROM tbl_ticket WHERE purchased_by = '$getUserID' AND DATE_FORMAT(purchased_date,'%Y') = '$currentYear' ORDER BY purchased_date DESC LIMIT 5
		");
	}
		...
		$qAccumulateTicketBought = mysqli_query($con, "SELECT COALESCE(SUM(total_ticket), 0) AS total_ticket FROM tbl_ticket WHERE purchased_by = '" . $getUserID . "' AND purchased_date BETWEEN '$startQuery' AND '$endQuery'");
		$dAccumulateTicketBought = mysqli_fetch_array($qAccumulateTicketBought);
		$totalTicket = $dAccumulateTicketBought['total_ticket'];
		...
		$qRafflesWon = mysqli_query($con, "SELECT ROUND(COALESCE(SUM(pot.total), 0) / 2, 2) AS total_won FROM tbl_ticket w JOIN (SELECT campaignid_fk, SUM(total_price) AS total FROM tbl_ticket GROUP BY campaignid_fk) pot ON pot.campaignid_fk = w.campaignid_fk WHERE w.purchased_by = '" . $getUserID . "' AND w.win = 'Y' AND w.purchased_date BETWEEN '$startQuery' AND '$endQuery'");
		$dRafflesWon = mysqli_fetch_array($qRafflesWon);
		$totalRafflesWon = $dRafflesWon['total_won'];
		
		$qTicketPurchased = mysqli_query($con, "
			SELECT campaignid_fk, DATE_FORMAT(purchased_date,'%m/%d/%Y') AS purchased_date, total_price, card_last4 AS card_number, payment_status FROM tbl_ticket WHERE purchased_by = '$getUserID' AND purchased_date BETWEEN '$startQuery' AND '$endQuery' ORDER BY purchased_date DESC LIMIT 5
		");
	}
