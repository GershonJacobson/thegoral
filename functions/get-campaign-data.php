<?php
	...
	if($filter == "" || $filter == "pmmtl") {
		$query = "
			...
			WHERE
			tbl_campaign.added_by = '$getUserID' AND
			tbl_campaign.category = ''
			GROUP BY tbl_campaign.campaign_id 
			ORDER BY tbl_campaign.status DESC, rating DESC limit ".$row.",".$rowperpage;
	}
		...
		$query = "
			...
			tbl_campaign.category = ''
			GROUP BY tbl_campaign.campaign_id 
			ORDER BY tbl_campaign.status DESC, rating ASC limit ".$row.",".$rowperpage;
	}
		...
		$query = "
			...
			tbl_campaign.added_by = '$getUserID' AND
			tbl_campaign.category = ''
			ORDER BY end_date ASC limit ".$row.",".$rowperpage;
	}
