<?php
	...
	if(!in_array((int)$getUserRole, [1, 2, 3], true)) {
	...
	if($filter == "" || $filter == "pmmtl") {
		...
		FROM tbl_campaign WHERE public = 1 AND ((status = 'open' AND keep_show = 0) OR (status = 'closed' AND keep_show = 1)) ORDER BY category DESC, end_date ASC limit ".$row.",".$rowperpage;
	}
		...
		DATE_FORMAT(start_date,'%H:%i') AS start_time,
		DATE_FORMAT(end_date,'%H:%i') AS end_time
		FROM tbl_campaign WHERE public = 1 AND ((status = 'open' AND keep_show = 0) OR (status = 'closed' AND keep_show = 1)) ORDER BY category DESC, end_date limit ".$row.",".$rowperpage;
	}
	...
	while($data = mysqli_fetch_array($result)){
		...
		array_push($list, (object)[
			...
			"campaignName" => htmlspecialchars($campaignName, ENT_QUOTES, 'UTF-8'),
			"startDate" => $startDate,
			"endDate" => $endDate,
			"firstName" => $firstName,
			"lastName" => $lastName,
